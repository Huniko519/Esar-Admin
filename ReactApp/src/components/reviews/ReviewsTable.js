...
import getColumns from './ReviewsColumns';
import Edit from "@material-ui/icons/Edit";
import { Switch } from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteReviews, getReviews} from "../../repo/reviewsRepo";
...
const ReviewsTable = (props) => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {   setLoading(true);
                const res = await getReviews(query.page,query.pageSize,query.search);
                setLoading(false);
                return ({
                    data: res.data,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/reviews/update/${rowData.id}`,
                        user:rowData
                    })
                }
            },
