...
import getColumns from './CurrenciesColumns';
import Edit from "@material-ui/icons/Edit";
import { Switch } from "@material-ui/core";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteCurrencies, getCurrencies} from "../../repo/currenciesRepo";
...
const CurrenciesTable = (props) => {
    ...
    <MaterialTable
        ...
        data={async(query)=>
            {   setLoading(true);
                const res = await getCurrencies(query.page,query.pageSize,query.search);
                setLoading(false);
                return ({
                    data: res.data,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/currencies/update/${rowData.id}`,
                        user:rowData
                    })
                }
            },
